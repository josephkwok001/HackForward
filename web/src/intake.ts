...
export function redactSensitive(text: string): { text: string; notice: string | null } {
  ...
  redacted = redacted.replace(/\b(?:\d[ -]*?){13,19}\b/g, () => {
    notices.push("card number");
    return "[redacted card number]";
  });

  if (/\b(otp|one[ -]?time|sms code|verification code|code)\b/i.test(redacted)) {
    redacted = redacted.replace(/\b\d{6}\b/g, () => {
      notices.push("OTP or verification code");
      return "[redacted code]";
    });
  }

  redacted = redacted.replace(/\bpassword\s*[:=]\s*\S+/gi, () => {
    notices.push("password");
    return "password: [redacted]";
  });

  const removed = unique(notices);
  return {
    text: redacted,
    notice: removed.length
      ? `Sensitive data was removed before intake: ${removed.join(", ")}.`
      : null,
  };
}
